...
frappe.provide("syncedoffice.crm");
syncedoffice.pre_sales.set_as_lost("Opportunity");
syncedoffice.sales_common.setup_selling_controller();
...
frappe.ui.form.on("Opportunity", {
	...
	party_name: function (frm) {
		...
		if (frm.doc.opportunity_from == "Customer") {
			syncedoffice.utils.get_party_details(frm);
		} else if (frm.doc.opportunity_from == "Lead") {
			syncedoffice.utils.map_current_doc({
				method: "syncedoffice.crm.doctype.lead.lead.make_opportunity",
	...
	customer_address: function (frm, cdt, cdn) {
		syncedoffice.utils.get_address_display(frm, "customer_address", "address_display", false);
	},

	contact_person: syncedoffice.utils.get_contact_details,
	...
	refresh: function (frm) {
		var doc = frm.doc;
		frm.trigger("setup_opportunity_from");
		syncedoffice.toggle_naming_series();
		...
		if (!frm.is_new() && doc.status !== "Lost") {
			...
			let company_currency = syncedoffice.get_currency(frm.doc.company);
	...
	currency: function (frm) {
		...
		if (company_currency != frm.doc.currency) {
			frappe.call({
				method: "syncedoffice.setup.utils.get_exchange_rate",
				args: {
	...
	make_supplier_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.opportunity.opportunity.make_supplier_quotation",
			frm: frm,
		});
	},
	...
	make_request_for_quotation: function (frm) {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.opportunity.opportunity.make_request_for_quotation",
			frm: frm,
		});
	},
...
syncedoffice.crm.Opportunity = class Opportunity extends frappe.ui.form.Controller {
	onload() {
	...
	setup_queries() {
		...
		me.frm.set_query("customer_address", syncedoffice.queries.address_query);

		this.frm.set_query("item_code", "items", function () {
			return {
				query: "syncedoffice.controllers.queries.item_query",
				filters: { is_sales_item: 1 },
			};
		});

		me.frm.set_query("contact_person", syncedoffice.queries["contact_query"]);

		if (me.frm.doc.opportunity_from == "Lead") {
			me.frm.set_query("party_name", syncedoffice.queries["lead"]);
		} else if (me.frm.doc.opportunity_from == "Customer") {
			me.frm.set_query("party_name", syncedoffice.queries["customer"]);
	...
	create_quotation() {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.opportunity.opportunity.make_quotation",
			frm: this.frm,
		});
	}
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.opportunity.opportunity.make_customer",
			frm: this.frm,
		});
	}

	show_notes() {
		const crm_notes = new syncedoffice.utils.CRMNotes({
			frm: this.frm,
	...
	show_activities() {
		const crm_activities = new syncedoffice.utils.CRMActivities({
			frm: this.frm,
...
extend_cscript(cur_frm.cscript, new syncedoffice.crm.Opportunity({ frm: cur_frm }));

cur_frm.cscript.item_code = function (doc, cdt, cdn) {
	var d = locals[cdt][cdn];
	if (d.item_code) {
		return frappe.call({
			method: "syncedoffice.crm.doctype.opportunity.opportunity.get_item_details",
