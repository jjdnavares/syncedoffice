...
frappe.provide("syncedoffice");
if (this.frm) {
	this.frm.email_field = "email_id";
}
syncedoffice.LeadController = class LeadController extends frappe.ui.form.Controller {
	setup() {
	...
	add_lead_to_prospect(frm) {
		frappe.prompt(
			...
			function (data) {
				frappe.call({
					method: "syncedoffice.crm.doctype.lead.lead.add_lead_to_prospect",
					args: {
						lead: frm.doc.name,
	...
	make_customer() {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.lead.lead.make_customer",
			frm: this.frm,
		});
	}
	...
	make_quotation() {
		frappe.model.open_mapped_doc({
			method: "syncedoffice.crm.doctype.lead.lead.make_quotation",
			frm: this.frm,
		});
	}
...
if (this.frm) {
	extend_cscript(this.frm.cscript, new syncedoffice.LeadController({ frm: this.frm }));
}
